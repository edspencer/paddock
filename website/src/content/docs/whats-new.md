...
- **Background tasks survive the turn boundary.** In session mode, work a keeper
  ...
  to continue or report back. No reload, and no more tasks quietly dying because
  *Paddock* tore the turn down. (The underlying runtime can still kill one at the
  boundary — that's what [keeper-chat recovery](/configuration/keeper-recovery/)
  is still there for.) This is what makes "kick off something slow, get pinged
  when it's done" reliable for keepers.
