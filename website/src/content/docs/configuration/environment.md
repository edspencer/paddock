...
| `CLAUDE_HOME` | `~/.claude` | no | The Claude home whose `projects/<encoded-cwd>/` folders hold Claude Code's session transcripts. Resolved **once** at startup (`claudeHome()` in `config.ts`) and threaded from there to *both* consumers: Paddock's own transcript relocation and chat-import detection (`ensureProjectChats` in `transcripts.ts`, `AdoptableIndex` in `adoptable.ts`), **and** the engine, as `FleetManagerOptions.claudeHomePath` (`herdctl.ts`) — which is in turn the home Claude Code itself reads and writes under. It is deliberately one value: were Paddock to honour this variable while the engine fell back to `$HOME/.claude`, chats would **list from one directory and open empty from another**. That divergence is invisible whenever `CLAUDE_HOME` already equals `$HOME/.claude`, which is most deployments — so it is threaded explicitly rather than left to a shared default. Point it at a mounted copy of the user's `~/.claude` when importing terminal chats into a containerised instance. |
