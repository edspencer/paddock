...
For a runnable starting point, copy
[`.env.example`](https://github.com/edspencer/paddock/blob/main/.env.example) to
`.env` and adjust. Authentication is summarised below but documented in full in
...
|----------|---------|----------|---------|
| `PADDOCK_PROFILE` | `balanced` | no | The security/capability posture this instance starts from: `paranoid` \| `balanced` \| `yolo`. Expands to defaults for the five `claude.*` sharing modes, `maxSpawnDepth` and the capability toggles — the variables marked *(profile: …)* below. Any of those set explicitly still wins; so does an individual key in the [config file](/configuration/config-file/), which is the one place Paddock's usual environment-beats-file rule inverts (*specific beats general*). Profiles never touch port, bind host, auth, models or drive mode — **`yolo` does not open the bind or relax auth.** An unknown name falls back to `balanced` rather than failing the boot. `paranoid` is exactly the behaviour Paddock had before profiles existed. See **[Config profiles](/configuration/profiles/)**. |
...
string (`command`, an `args` entry, an `env` value, `url`, a `headers` value),
`env:VAR_NAME` reads that value from the environment instead:
...
| `PADDOCK_SELF_MCP` | *(profile: on from `balanced` up)* | no | Give Claude the read-only self-management MCP (`mcp__paddock_manage__*`: enumerate projects/chats, read another chat's transcript). |
| `PADDOCK_SELF_MCP_WRITE` | *(profile: `yolo` only)* | no | Additionally give Claude the self-management **write** tools (`create_chat`, `fork_chat`, `send_message`, `fork_chat_batch`). Only honored when `PADDOCK_SELF_MCP` is also on (write implies read). |
| `PADDOCK_SELF_MCP_PROJECTS` | *(profile: `yolo` only)* | no | Additionally give Claude the self-management **project** tools (`create_project`, `promote_project`) — provisioning a whole new project, or promoting an existing **managed** (notebook) project into an unmanaged one backed by a repo — cloning a caller-supplied URL either way. Gated separately from the other write tools because it creates instance-level state and clones a caller-supplied git URL. Only honored when `PADDOCK_SELF_MCP` and `PADDOCK_SELF_MCP_WRITE` are also on. |
| `PADDOCK_MAX_SPAWN_DEPTH` | *(profile: `1`; `2` on `yolo`)* | no | How deep a spawn tree may grow before spawned children stop receiving the self-management MCP: a spawned turn at depth `d` gets it (including the write tools, so a child can `send_message` back to its parent) only while `d ≤` this value. `0` means no spawned child ever gets it. A per-project `maxSpawnDepth` overrides this at dispatch; an out-of-range value falls back to the default rather than failing startup. Only meaningful when the **write** self-MCP is on — spawning needs those tools. |
| `PADDOCK_SCHEDULE_MUTATION` | *(profile: `yolo` only)* | no | Allow schedules to be created / edited / deleted **programmatically** at runtime (the Schedules REST routes and the trigger MCP tools). Off by default, so a plain instance's schedules can only change by editing `project.yaml`. Schedules declared statically in `project.yaml` are armed either way. Accepts `1`/`true`/`yes`. See [Scheduling & the schedule gates](/configuration/schedules/). |
| `PADDOCK_HOOKS_MCP` | *(profile: `yolo` only)* | no | Instance default for the hook/trigger-management tools (`list_triggers` / `set_trigger` / `remove_trigger`) — Claude declaring and editing its own [event hooks](/concepts/hooks/) and schedules. Off by default; a per-project `hooksMcpEnabled` in `project.yaml` overrides it. Only honored when the self-management **write** MCP is also on; when off the tools are **absent** (not present-but-refusing). Accepts `1`/`true`/`yes`. |
...
| `PADDOCK_BROWSER_MCP` | *(profile: `yolo` only)* | no | When `=1`, inject a headless-Chromium Playwright MCP into the agent (browse/screenshot). |
