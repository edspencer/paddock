...
Earlier versions of this page listed them as exempt, but no route was ever
registered — so with the SPA mounted they were answered by the front-end catch-all
with `200 text/html`, and a probe pointed at one reported healthy no matter what
state the app was in. They are now gated like any other unknown path (`401` in
`trusted-header`/`jwt` mode). If you have a probe on one of them, repoint it at
`/api/health` ([issue #569](https://github.com/edspencer/paddock/issues/569)).
:::
