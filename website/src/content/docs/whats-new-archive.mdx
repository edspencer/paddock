---
title: What's New — earlier releases
description: "The full archive of Paddock release highlights, 0.58 and earlier — CLI output and flags, the environment system prompt, the npm package, adopting Claude Code CLI chats, Home's attention feeds, subtree actions on the chat tree, the root as a first-class workspace, scratch retired, an external MCP endpoint and generated API reference, per-message fork and revert, Opus 5 and a configurable model list, official Docker images, background work that outlives the turn, attachments, streaming, unified triggers, event hooks, schedules and run history, and first-class MCP tool rendering."
---

import DemoVideo from '../../components/DemoVideo.astro';

The long tail of [What's New](/whats-new/), kept in full. Entries here describe
releases **as they shipped** — several have since been superseded, and where that
matters the entry says so.

Nothing is removed from this page; it grows from the bottom as the main page is
trimmed.
