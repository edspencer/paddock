...
Claude Code doesn't just reply — it *works*: it reads files, edits them, runs
...
renders each tool call as a compact, expandable block, and gives you two
dedicated tabs — **Files** and **Changes** — for inspecting and committing what
...
Every tool Claude calls becomes a collapsible block in the transcript. The header
always shows a per-tool icon, the tool name, a short subtitle (usually the
...
was read, e.g. **`lines 40–71 of 210`**. When Claude reads an **image** that lives
inside the project directory, the block renders the picture **inline**
...
Since **v0.43**, work Claude kicks off in the background on a session-mode chat
...
ends, leaving Claude idle with nothing to wake it. That's what
[chat recovery](/configuration/chat-recovery/) surfaces and re-drives, and it's
still shipping and still relevant.
:::
...
When Claude uses Paddock's *own* management tools — the `mcp__…` tools it can be
...
an instance (and, for spawned chats, within a bounded depth). Most chats won't
...
- **Cost** is the chat's cumulative token usage and an estimated dollar figure,
  **including tokens spent by any sub-agents** Claude spawned. The dollar number
  is a **ballpark at standard API list prices** — a sense of scale, not a
...
The **Files** tab lists the project's working directory, one level at a time.
Subdirectories Claude filed things under — a `docs/`, `design/`, or `src/` — are
...
For a project whose store is a git repo, the **Changes** tab is where you turn
Claude's edits into commits. It lists every uncommitted file with its status
...
a review tells you Claude went wrong at an identifiable step — rewind to just
...
*conversation*, not the files, PRs or messages Claude already produced.
