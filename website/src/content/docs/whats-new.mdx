...
- **`profile: paranoid | balanced | yolo` sets an instance's security posture in
  ...
  ![The foot of the Config screen: an "Advanced (read-only)" group headed by a Posture profile row marked READ-ONLY, its value reading "balanced", above help text saying the profile sets the default for the Claude sharing modes and most of the Capabilities toggles, and that anything set explicitly in the file or the environment still wins over it](../../assets/whats-new/whatsnew-posture-profile.png)
