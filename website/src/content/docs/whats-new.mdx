---
title: What's New
description: "User-facing highlights of each Paddock release, newest first — queued messages, the Config screen redesign and the new default port, linked project directories, host Claude Code plugins, granular inheritance from your Claude Code state, adopting terminal chats, and the npm package. Earlier releases are archived on a companion page."
---
...
- **A new instance opens on Discover** instead of an empty project list. It reads
  ...
  apart. Tick the ones you want, press Import, and each becomes a project
  pointing at that directory with its conversations copied in as resumable chats.
  It stays in the sidebar afterwards; it is not only a first-run screen.
...
- **Importing a directory writes nothing into it.** No `.paddock/`, no `.chats/`,
  no `.gitignore` edit, no `CLAUDE.md` — the project record and the copied
  transcripts both live in the data dir, and the project simply points at the
  path. Your `~/.claude` transcripts are copied with their timestamps preserved,
  never moved or deleted, so your terminal `claude` is unaffected.
