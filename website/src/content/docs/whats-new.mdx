...
- **`--here` opens the directory you're standing in.** Run
  ...
  `.paddock/` for the workspace's state and `.chats/` for transcripts, adds both to
  your `.gitignore`, and **links** your `~/.claude` history rather than moving or
  copying it. Later runs in the same directory resume it with no flag needed — the
  git model, where `--here` is `git init` and `.paddock/` is `.git`. Without the
  flag, Paddock never touches the directory you ran it from.
