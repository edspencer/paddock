...
- **Paddock no longer plants anything in a Claude home it doesn't own.** It used
  ...
  It skipped directories you already had history in, but not empty ones — which is
  exactly what `--here` is usually pointed at. From then on every `claude` session
