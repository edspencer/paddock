...
// ── Local (client/TUI) slash-command output & caveat (issue #158) ────────────
//
// The client-local display commands `/context` and `/usage` (and friends) write
// two more `type:"user"`-shaped artifacts that CC never asked the human to type:
//
//   1. a `<local-command-caveat>Caveat: … DO NOT respond …</local-command-caveat>`
//      framing note — pure harness noise, no value to a reader; and
//   2. a `<local-command-stdout>…</local-command-stdout>` block carrying the
//      command's ACTUAL rendered output (e.g. `/context`'s usage table).
//
// In current Claude Code the stdout lands in a `type:"system"` / `local_command`
// transcript entry that @herdctl/core's parser drops (it keeps only user/assistant
// entries) and @herdctl/chat's translator skips (it's delivered live as a
// `model:"<synthetic>"` assistant message it treats as a placeholder) — so the
// output silently vanishes in both the live and history paths (issue #158). We
// surface it Paddock-side: the live path (ws.ts) re-emits the `local_command`
// entry as an assistant note, and the history path detects these two artifacts
// here so the stdout renders as a clean, labeled output block (not a raw-XML user
// bubble) and the caveat is dropped. Detection is forgiving of leading whitespace.

/**
 * True when a `role:"user"` message is CC's `<local-command-caveat>` framing note
 * ("Caveat: The messages below were generated by the user while running local
 * commands. DO NOT respond…"). It's harness scaffolding with no reader value, so
 * the renderer drops it rather than showing a raw-XML user bubble (issue #158).
 * (CC usually flags it `isMeta:true` so @herdctl/core drops it upstream; this is a
 * defensive guard for versions/paths where it leaks through unflagged.)
 */
export function isLocalCommandCaveat(content: string): boolean {
  return content.trimStart().startsWith("<local-command-caveat>");
}

/**
 * If a `role:"user"` message is a `<local-command-stdout>…</local-command-stdout>`
 * block (the rendered output of a client-local command like `/context` / `/usage`),
 * return its INNER text so the renderer can show it as a labeled command-output
 * block instead of a raw-XML user bubble (issue #158); otherwise null. A block with
 * no inner text (a display-only command that produced nothing) yields null so it
 * collapses away rather than rendering an empty bubble.
 */
export function localCommandStdout(content: string): string | null {
  const m = /^\s*<local-command-stdout>([\s\S]*)<\/local-command-stdout>\s*$/.exec(content);
  if (!m) return null;
