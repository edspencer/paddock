...
export function ProjectView() {
  ...
  // Partition the chat list into the current (top) and archived (bottom) groups.
  const activeChats = chats.filter((c) => !c.archived);
  const archivedChats = chats.filter((c) => c.archived);
  const activeIsArchived = archivedChats.some((c) => c.sessionId === activeSession);
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="mb-1 flex items-center justify-between pr-3">
              ...
              {activeChats.length > 0 && (
                <span className="text-[11px] text-paddock-400">{activeChats.length}</span>
              )}
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-2">
              ...
              {chats.length > 0 && activeChats.length === 0 && (
                <p className="px-2 py-2 text-sm text-paddock-500">
