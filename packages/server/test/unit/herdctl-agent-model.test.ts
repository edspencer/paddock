/**
 * HerdctlService.ensureAgentModel — the per-chat model-override primitive the
 ...
 *   1. ensureAgentModel re-registers the keeper at the requested model, and
...
describe("HerdctlService.ensureAgentModel (per-chat model override — #336)", () => {
  it("re-registers the keeper agent at the requested model", async () => {
    const { svc, added } = svcWithFakeFleet();
    await svc.ensureAgentModel(project(), "claude-sonnet-5");
  ...
  it("re-registers again when the model actually changes", async () => {
    const { svc, added } = svcWithFakeFleet();
    await svc.ensureAgentModel(project(), "claude-sonnet-5");
    await svc.ensureAgentModel(project(), "claude-haiku-4-5-20251001");
