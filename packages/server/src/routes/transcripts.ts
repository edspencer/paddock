          ...
          200: {
            ...
            properties: {
              ...
              restartRequired: {
                type: "boolean",
                description:
                  "Whether a restart is still needed for the on-disk state to take effect, and until it happens the chat list stays BLANK — the completion screen must say so. On a FLIP that is because config is frozen at boot (app.ts:128), so the running process is still resolving `own` against a `.chats/` this call just emptied. On a stranded-`host` RECOVERY the process already resolves `host`; the blank list is instead because the redirect symlink is only planted at boot, by the `pointChatsDirAt` call that this run's empty `.chats/` finally lets succeed. Either way the restart is what makes the chats appear. TRUE for a repeat POST that wrote nothing but whose earlier write has not been restarted into yet; the design's 'always true when configWritten' would have reported false there, and would report false for every recovery.",
              },
            },
          },
          400: {
