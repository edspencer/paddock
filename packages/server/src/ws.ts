        ...
        if (slug !== SCRATCH_SLUG && deps.cfg.selfMcpEnabled) {
          ...
          if (deps.cfg.selfMcpWriteEnabled) {
            ...
            writeCtx = {
              ...
              // C1 (#263). Archive/unarchive is presentational metadata only — no
              // turn is started — so it delegates straight to the ArchiveStore,
              // keyed by the target project's agent (mirrors the POST archive
              // endpoints in routes.ts). Enables the "work → archive myself on
              // success" self-reporting convention. `deps.projects.get` validates
              // the slug (throws not_found), matching the other write callbacks.
              setArchived: async (projectSlug, targetSessionId, archived) => {
                await deps.projects.get(projectSlug);
                await deps.archive.setArchived(
