...
> below are current unless a callout says otherwise; where the 5.10.1 answer is
> load-bearing history it is kept in a *Superseded* note rather than deleted.
>
...
const session: RuntimeSession = await fleet.openChatSession("keeper-foo", {
  ...
  injectedMcpServers,       // in-process SDK MCP servers (see ARCHITECTURE.md §5)
  systemPromptAppend,
...
> **`schedule:skipped` is NOT a FleetManager event** — it lives on
> `JobQueueEventMap` (`fleet-manager/job-queue.d.ts:242`). An earlier revision of
> this page listed it here; subscribing on the FleetManager gets you nothing.
...
- Runtime/drive-mode split: [`ARCHITECTURE.md`](./ARCHITECTURE.md) §9.
